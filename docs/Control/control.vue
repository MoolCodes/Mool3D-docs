<template>
  <div>
    ...
    <div class="btn">
      <button class="custom-btn btn-12" @click="setOrbit">
        <span>Click!</span><span>轨道控制器</span>
      </button>
      <button class="custom-btn btn-12" @click="setTranform">
        <span>Click!</span><span>变换控制器</span>
      </button>
    </div>
  </div>
</template>
...
const setOrbit = () => {
  sceneA.controls.transformControls.detach();
  sceneA.controls.transformControls.enabled = false;
  sceneA.controls.orbitControls.enabled = true;
};

const setTranform = () => {
  sceneA.controls.transformControls.enabled = true;
  sceneA.controls.orbitControls.enabled = false;
  sceneA.controls.transformControls.detach();
};
//切换场景
